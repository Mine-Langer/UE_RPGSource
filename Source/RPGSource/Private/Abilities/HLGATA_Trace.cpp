...
#include "Abilities/HLGATA_Trace.h"
...
void AHLGATA_Trace::BeginPlay()
{
	Super::BeginPlay();

	// 先禁用tick 之后在StartTargeting中启用，在StopTargeting中禁用
	SetActorTickEnabled(false);
}

void AHLGATA_Trace::Tick(float DeltaTime)
{
...
void AHLGATA_Trace::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void AHLGATA_Trace::StartTargeting(UGameplayAbility* Ability)
{
	// Super::StartTargeting(Ability); // Don't call to Super because we can have more than one Reticle
...
void AHLGATA_Trace::ConfirmTargetingAndContinue()
{
...
void AHLGATA_Trace::CancelTargeting()
{
...
void AHLGATA_Trace::ResetSpread()
{
...
float AHLGATA_Trace::GetCurrentSpread() const
{
...
void AHLGATA_Trace::SetShouldProduceTargetDataOnServer(bool bInShouldProduceTargetDataOnServer)
{
	ShouldProduceTargetDataOnServer = bInShouldProduceTargetDataOnServer;
}

void AHLGATA_Trace::LineTraceWithFilter(TArray<FHitResult>& OutHitResults, const UWorld* World,
...
void AHLGATA_Trace::AimWithPlayerController(const AActor* InSourceActor, FCollisionQueryParams Params,
...
bool AHLGATA_Trace::ClipCameraRayToAbilityRange(FVector CameraLocation, FVector CameraDirection, FVector AbilityCenter,
	float AbilityRange, FVector& ClippedPosition)
{
	...
	if (DotToCenter >= 0)
	{
		float DistanceSquard = CameraToCenter.SizeSquared() - (DotToCenter*DotToCenter);
		float RadiusSquared = AbilityRange*AbilityRange;
		if (DistanceSquard <= RadiusSquared)
		{
			float DistanceFromCamera = FMath::Sqrt(RadiusSquared - DistanceSquard);
...
void AHLGATA_Trace::StopTargeting()
{
	SetActorTickEnabled(false);
...
void AHLGATA_Trace::SetStartLocation(const FGameplayAbilityTargetingLocationInfo& InStartLocation)
{
	StartLocation = InStartLocation;
}

void AHLGATA_Trace::SetDestroyOnConfirmation(bool bInDestroyOnConfirmation)
{
	bDestroyOnConfirmation = bInDestroyOnConfirmation;
}


FGameplayAbilityTargetDataHandle AHLGATA_Trace::MakeTargetData(const TArray<FHitResult>& HitResults) const
{
	FGameplayAbilityTargetDataHandle RetDataHandle;

	for (int32 i=0; i<HitResults.Num(); i++)
	{
		// 这些是由FGameplayAbilityTargetDataHandle（通过内部TSharedPtr）清理
		FGameplayAbilityTargetData_SingleTargetHit* ReturnData = new FGameplayAbilityTargetData_SingleTargetHit();
		ReturnData->HitResult = HitResults[i];
		RetDataHandle.Add(ReturnData);
	}
	
	return RetDataHandle;
}

TArray<FHitResult> AHLGATA_Trace::PerformTrace(AActor* InSourceActor)
{
	...
	// FCollisionQueryParams 是用于指定碰撞查询相关设置的结构体，用来调整碰撞检测时的行为
	FCollisionQueryParams Params(SCENE_QUERY_STAT(AHLGATA_LineTrace), bTraceComplex);
	Params.bReturnPhysicalMaterial = true; // 射线检测的返回结果中会包含被命中的物体的物理材质
	Params.AddIgnoredActors(ActorsToIgnore); // 用于向 Params 中添加需要在碰撞检测中忽略的Actor
	Params.bIgnoreBlocks = bIgnoreBlockingHits; // 决定是否忽略“阻挡”的物体
	...
	if (MasterPC)
	{
		...
		MasterPC->GetPlayerViewPoint(ViewStart, ViewRotation);
	...
	{
		AimWithPlayerController(InSourceActor, Params, TraceStart, TraceEnd); // 仅对服务器和启动客户端有效
...
AGameplayAbilityWorldReticle* AHLGATA_Trace::SpawnReticleActor(FVector Location, FRotator Rotation)
{
	if (ReticleClass)
	{
...
void AHLGATA_Trace::DestroyReticleActors()
{
